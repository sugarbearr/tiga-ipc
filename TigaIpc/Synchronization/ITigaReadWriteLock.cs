...
public interface ITigaReadWriteLock : IDisposable
{
    /// <summary>
    /// Gets a value indicating whether is true if a read lock is being held
    /// </summary>
    bool IsReaderLockHeld { get; }

    /// <summary>
    /// Gets a value indicating whether is true if a write lock (which means all read locks) is being held
    /// </summary>
    bool IsWriterLockHeld { get; }

    /// <summary>
    /// Gets the name of the lock if it was created with a name
    /// </summary>
    string? Name { get; }
